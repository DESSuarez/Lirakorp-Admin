...
export default function ContractFileUpload({ contractId, existingFileUrl, existingFileName }: ContractFileUploadProps) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await fetch(`/api/contracts/${contractId}/upload-file`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      {existingFileUrl && existingFileName ? (
        <div className="space-y-2">
          <div className="flex items-center gap-2 p-2 bg-green-50 border border-green-200 rounded-lg">
            <span className="text-lg">{existingFileName.endsWith('.pdf') ? '📕' : '📘'}</span>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-gray-900 truncate">{existingFileName}</p>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex-1 text-center px-2 py-1.5 text-xs font-medium text-red-600 bg-white border border-red-200 rounded-lg hover:bg-red-50 transition-colors"
            >
              Eliminar
            </button>
          </div>
        ...
        <label className={`flex items-center gap-3 p-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-blue-400 hover:bg-blue-50/50 transition-all ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-gray-700">{uploading ? 'Subiendo...' : 'Subir contrato firmado'}</p>
